...
# Injected on every new page — covers the main fingerprinting vectors Google uses
_STEALTH_JS = """
// Hide automation flag
Object.defineProperty(navigator, 'webdriver', {get: () => undefined});

// Fake a real plugin list (headless has 0 plugins)
const pluginData = [
...
// Chrome object (missing in Playwright headless)
if (!window.chrome) window.chrome = {};
window.chrome.runtime = {};

// Notifications permission (avoid the detectable query pattern)
...
// WebGL vendor — headless reports a generic string
...
// Battery API — return plausible values instead of failing
...
// Realistic screen properties (headless defaults to 800x600)
...
async def _session_warmup(page: Page) -> None:
    """Visit google.com briefly to establish fresh cookies before hitting Maps."""
    try:
...
async def scrape(
    ...
    async with async_playwright() as pw:
        ...
        clean_streak = 0  # consecutive successful pages — used to speed up after warmup
        ...
        while True:
            ...
            # Adaptive delay: start cautious, ease up after 5 clean pages, slow down on errors
            if clean_streak < 5:
                delay = random.uniform(1.2, 2.8)
            elif clean_streak < 20:
